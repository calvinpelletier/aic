...
def _count_early_ply(trim):
    n_early = 0
    for i, batch in enumerate(_data_iter('b2a', trim=trim, include_ply=True)):
        if i >= 200:
            break
...
def _count_low_elo(drop):
    n_low = 0
    for i, batch in enumerate(_data_iter('bm2o', drop=drop)):
        if i >= 200:
            break
...
def _data_iter(task, trim=True, drop=True, include_ply=False):
    return train_data_iter(_cfg(task, trim, drop), DEVICE, 0, include_ply)
